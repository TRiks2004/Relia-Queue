...
def run_simulation(T: float, num_channels: int, service_time: float, num_iterations: int, alfa: int):
    ...
    * `NumChannelsNegative` - Если `num_channels` меньше нуля.
    ...
    * `AlphaIsZero` - Если `alfa` равно нулю.
    ...
    params = SimulationParameters(T=T, num_channels=num_channels, service_time=service_time, alfa=alfa)
    results = []

    # Запуск итераций симуляции
