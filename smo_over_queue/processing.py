...
def process_iteration(service_time: float, max_time: float, alpha: float, num_threads: int) -> Answer:
    ...
    * `num_threads (int)` - Количество потоков (серверов) для обработки заявок.
    ...
    ### Описание:
   Данный код представляет функцию process_iteration, которая симулирует процесс обработки заявок в системе массового обслуживания (СМО) с неограниченной очередью. Вот краткое описание ключевых шагов, выполняемых в этой функции:

    1. Создается список iterations для хранения данных каждой итерации симуляции.
    2. Инициализируются переменные, такие как текущее время current_time, случайное значение random_value, интервал между заявками interval, список серверов servers и счетчики.
    3. Затем выполняется основной цикл, в котором происходит моделирование обработки заявок:
    - Для каждой итерации рассчитываются интервалы между поступлением заявок, время обслуживания и распределение заявок по серверам.
    - Новые заявки назначаются свободным серверам или ждут, пока освободится хотя бы один из серверов.
    - Данные каждой итерации сохраняются в объекте Iteration и добавляются в список iterations.
    - Генерируется новое случайное значение для следующего интервала между заявками.
    4. Функция возвращает объект Answer, содержащий список итераций и количество обработанных заявок.
    """
